...
router.post('/signup', (req, res, next) => {
  if (req.headers.authorization === 'nUwdaa*=632Ate') {
    if (typeof req.body.firstName === 'string' &&
      ...
      let sql = `INSERT INTO users (first_name, last_name, created_on, email, username, salt, hashed_pass, role, count_assigns) VALUES ("${user.firstName}", "${user.lastName}", NOW(), "${user.email}", "${user.username}", "${user.salt}", "${user.hashedPass}", "${user.role}", 0)`
